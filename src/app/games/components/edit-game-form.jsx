...
import { updateGameSessionAction, } from '#app/lib/actions/game-sessions';
import { UpdateGameSessionSchema } from '#app/lib/validations/game-session';
export var EditGameForm = function (_a) {
    var _b;
    var gameSession = _a.gameSession, allPlayers = _a.allPlayers, updateOptimisticGameSession = _a.updateOptimisticGameSession;
    var _c = useState(false), open = _c[0], setOpen = _c[1];
    var _d = useActionState(function (prevState, formData) { return __awaiter(void 0, void 0, void 0, function () {
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    return [4 /*yield*/, updateGameSessionAction(prevState, formData)];
                case 1: return [2 /*return*/, _a.sent()];
            }
        });
    }); }, undefined), lastResult = _d[0], formAction = _d[1], isSubmitting = _d[2];
    var _e = useForm({
        lastResult: lastResult === null || lastResult === void 0 ? void 0 : lastResult.result,
        defaultValue: {
            id: gameSession.id,
            gameDatetime: gameSession.gameDatetime.toISOString(),
            description: (_b = gameSession.description) !== null && _b !== void 0 ? _b : '',
            playerIds: gameSession.players.map(function (player) { return player.id; }),
        },
        onValidate: function (_a) {
            var formData = _a.formData;
            return parseWithZod(formData, { schema: UpdateGameSessionSchema });
        },
        shouldValidate: 'onBlur',
        shouldRevalidate: 'onInput',
    }), form = _e[0], fields = _e[1];
    // Close dialog on successful submission
    useEffect(function () {
        if ((lastResult === null || lastResult === void 0 ? void 0 : lastResult.result.status) === 'success') {
            setOpen(false);
        }
    }, [lastResult]);
    return (<>
			<Button variant="ghost" size="sm" onClick={function () { return setOpen(true); }} className="w-full justify-start">
						...
						<FieldGroup>
							...
							<Field role="group" aria-labelledby={fields.playerIds.id}>
								...
								<div className="grid grid-cols-2 gap-2">
									{allPlayers.map(function (player) { return (<div key={player.id} className="flex items-center gap-2">
											<Checkbox id={"".concat(fields.playerIds.id, "-").concat(player.id)} name={fields.playerIds.name} value={player.id} defaultChecked={gameSession.players.some(function (p) { return p.id === player.id; })}/>
											<label htmlFor={"".concat(fields.playerIds.id, "-").concat(player.id)}>
												{player.name}
											</label>
										</div>); })}
								</div>
