...
2. View Description: <view_description> {{view-description}} <- paste
   description of the view being implemented from ui-plan.md </view_description>

3. User Stories: <user_stories> {{user-stories}} <- paste user stories from
   @prd.md that will be addressed by the view </user_stories>

4. Server Action Description: <server_action_description>
...
5. Server Action Implementation: <server_action_implementation>
   {{server-action-implementation}} <- replace with reference to server action
   implementations that the view will use (e.g., @src/lib/actions/players.ts,
   @src/lib/actions/teams.ts) </server_action_implementation>

6. Tech Stack: <tech_stack> {{tech-stack}} <- replace with reference to
   @tech-stack.md file </tech_stack>
...
3. List all needed main components, along with a brief description of their
   purpose, needed types, handled events, and validation conditions (remember to
   maximize usage of components from Shadcn UI)
...
7. List required server actions and their usage in components
...
- Handled events
- Validation conditions (detailed conditions, according to server actions)
...
@docs/frontend/views/{view-name}-view-implementation-plan.md. Do not include any
analysis and planning in the final output.
...
@docs/frontend/views/{view-name}-view-implementation-plan.md file and should not
duplicate or repeat any work done in the implementation breakdown. Be extremely
